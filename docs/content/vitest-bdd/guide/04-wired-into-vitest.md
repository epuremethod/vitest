...
sort: 4
refs: [vitest-bdd, options-type]
---

vitest-bdd is not a test framework. Vitest is the framework; vitest-bdd is a Vite plugin that teaches it to read Gherkin. The whole installation is one import and one line in `test.include`:

...
This chapter is for the person deciding whether vitest-bdd belongs in their stack. There is no code in it.
...
In the [épure](https://epuremethod.com) method, feature files are written *before* the code, by a domain expert and a method expert together, and signed by the people who own the need. From that moment the contract governs the whole structure: the AI implements against it, the senior reviews against it, and vitest-bdd is what makes the governing literal — every scenario runs green or the window does not close. [tilia](https://tiliajs.com) is the foundation sheet, [@tilia/query](https://tiliajs.com) the data sheet; this is the proof sheet, and this guide is the last panel of the drawing.

### How this guide works

Readers of the [tilia](https://tiliajs.com) and @tilia/query guides know Alice and her spaced-repetition flashcards. Her app has grown up: a language school uses it now, and the scheduling rules — when a failed card returns, how intervals stretch — are no longer hers to decide.

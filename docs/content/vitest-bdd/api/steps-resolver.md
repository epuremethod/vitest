...
Pass your own function as the `stepsResolver` option of [vitestBdd](api.html#vitest-bdd) to change the convention; return `null` to let the default error surface. The resolver is exported so a custom one can fall back on it.

```typescript
import { defineConfig } from "vitest/config";
import { stepsResolver, vitestBdd } from "vitest-bdd";
